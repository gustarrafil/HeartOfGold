...
int conta_nodos_arvbin  (Nodo_AB   *AB);// -- conta quant. de nós na arvore
...
int predecessor_arvbin  (Nodo_AB   *AB, Tipo_Dado Dado_in, Tipo_Dado *Dado_out){
	...
	if(AB == NULL){
		return 1;//vazio ou não encotrado
	}
...
int menor_arvbin(Nodo_AB *AB, Tipo_Dado Dado, Tipo_Dado *menor){
	...
	if(AB->FilhoEsq != NULL || AB->Dado > Dado){//caso não haja mais folhas as esquerdas
...
int conta_nodos_arvbin(Nodo_AB *AB){
	...
	if(AB->FilhoDir == NULL &&  AB->FilhoEsq == NULL){
		return 1;//caso não haja mais folhas
	}
...
int apaga_arvbin(Nodo_AB **AB){
	...
	while(percorre != NULL){
		...
		if(percorre->FilhoEsq == NULL && percorre->FilhoDir == NULL){
			...
			//verifica qual lado é oriundo para anular o valor
